...
namespace {
  ...
  struct Skill {
    ...
    Move pick_best(size_t multiPV, ThreadPool *threads);
  ...
  uint64_t perft(Position& pos, Depth depth) {
    ...
    for (const auto& m : MoveList<LEGAL>(pos))
    {
        ...
        if (Root)
            sync_thread_out(pos.threads()) << UCI::move(m, pos.is_chess960()) << ": " 
              << cnt 
              << sync_thread_endl(pos.threads());
    }
    return nodes;
  }
...
void Search::clear(ThreadPool* threads) {
...
void MainThread::search() {
  ...
  if (rootMoves.empty())
  {
      ...
      sync_thread_out(threads) << "info depth 0 score "
  ...
  if (bestThread != this)
      sync_thread_out(threads) << UCI::pv(bestThread->rootPos, bestThread->completedDepth, threads) << sync_thread_endl(threads);

  sync_thread_out(threads) << "bestmove " << UCI::move(bestThread->rootMoves[0].pv[0], rootPos.is_chess960());

  if (bestThread->rootMoves[0].pv.size() > 1 || bestThread->rootMoves[0].extract_ponder_from_tt(rootPos))
      *threads->io()->out() << " ponder " << UCI::move(bestThread->rootMoves[0].pv[1], rootPos.is_chess960());

  *threads->io()->out() << sync_thread_endl(threads);
}
...
void Thread::search() {
  ...
  size_t multiPV = size_t((*threads()->options())["MultiPV"]);
  Skill skill((*threads()->options())["Skill Level"], (*threads()->options())["UCI_LimitStrength"] ? int((*threads()->options())["UCI_Elo"]) : 0);
         ...
         && !(threads()->limits()->depth && mainThread && rootDepth > threads()->limits()->depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !threads()->stop; ++pvIdx)
      {
          ...
          while (true)
          {
              ...
              if (   mainThread
                  ...
                  && time->elapsed(threads()) > 3000)
                  sync_thread_out(threads()) << UCI::pv(rootPos, rootDepth, threads()) 
                    << sync_thread_endl(threads());
          ...
          if (    mainThread
              && (threads()->stop || pvIdx + 1 == multiPV || time->elapsed(threads()) > 3000))
              sync_thread_out(threads()) << UCI::pv(rootPos, rootDepth, threads()) << sync_thread_endl(threads());
      }
          ...
          && !mainThread->stopOnPonderhit)
      {
          ...
          if (time->elapsed(threads()) > totalTime)
          {
              ...
              if (mainThread->ponder)
                  ...
                  threads()->stop = true;
                   ...
                   && time->elapsed(threads()) > totalTime * 0.50)
  ...
  if (skill.enabled())
      std::swap(rootMoves[0], *std::find(rootMoves.begin(), rootMoves.end(),
                skill.best ? skill.best : skill.pick_best(multiPV, threads())));
}
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    posKey = pos.key();
    tte = threads->tt()->probe(posKey, ss->ttHit);
    ...
    if (!rootNode && !excludedMove && stb->Cardinality)
    {
            ...
            && !pos.can_castle(ANY_CASTLING))
        {
            ...
            if (err != TB::ProbeState::FAIL)
            {
                ...
                {
                    tte->save(threads->tt(), posKey, value_to_tt(value, ss->ply), ss->ttPv, b,
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= probCutBeta)
                {
                  ...
                  tte->save(threads->tt(), posKey, value_to_tt(value, ss->ply), ss->ttPv, BOUND_LOWER, depth - 3, move, ss->staticEval);
                  return value;
                }
            }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (rootNode && thisThread == threads->main() && time->elapsed(threads) > 3000)
          sync_thread_out(threads) << "info depth " << depth
                    << " currmove " << UCI::move(move, pos.is_chess960())
                    << " currmovenumber " << moveCount + thisThread->pvIdx << sync_thread_endl(threads);
      if (PvNode)
          (ss+1)->pv = nullptr;
      ...
      prefetch(threads->tt()->first_entry(pos.key_after(move)));
    ...
    */
    ...
    if (!excludedMove && !(rootNode && thisThread->pvIdx))
        tte->save(threads->tt(), posKey, value_to_tt(bestValue, ss->ply), ss->ttPv,
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    posKey = pos.key();
    tte = thisThread->threads()->tt()->probe(posKey, ss->ttHit);
    ttValue = ss->ttHit ? value_from_tt(tte->value(), ss->ply, pos.rule50_count()) : VALUE_NONE;
    ...
    else
    {
        ...
        if (bestValue >= beta)
        {
            ...
            if (!ss->ttHit)
                tte->save(thisThread->threads()->tt(), posKey, value_to_tt(bestValue, ss->ply), false, BOUND_LOWER,
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
        ...
        // Speculative prefetch as early as possible
        prefetch(thisThread->threads()->tt()->first_entry(pos.key_after(move)));
    ...
    // Save gathered info in transposition table
    tte->save(thisThread->threads()->tt(), posKey, value_to_tt(bestValue, ss->ply), pvHit,
...
string UCI::pv(const Position& pos, Depth depth, ThreadPool* threads) {

  std::stringstream ss;
  TimePoint elapsed = threads->time()->elapsed(threads) + 1;
  ...
  for (size_t i = 0; i < multiPV; ++i)
  {
      ...
      if ((*threads->options())["UCI_ShowWDL"])
...
bool RootMove::extract_ponder_from_tt(Position& pos) {
    ...
    pos.do_move(pv[0], st);
    Thread *thisThread = pos.this_thread();
    TTEntry* tte = thisThread->threads()->tt()->probe(pos.key(), ttHit);
